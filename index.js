...
class ExternalSort {
	...
	_WriteIntoTmpFile(data, i, comparer) {
		// Разделяем запись чанка на итерации, что бы не нагружать память
		let itertatios = Math.ceil(this.heatSize / 3);
		data = data.sort(comparer);

		return new Promise((res) => {
			const writer = fs.createWriteStream(`${this.chunkDir}/chunk_${i}.tmp`, {
				highWaterMark: Math.ceil(this.heatSize / itertatios),
			});
			...
			writer.on('drain', () => {
				...
				if ((--itertatios > 0 && data.length > 0) || data.length > 0) write();
				else res();
			});
	...
	async _SplitIntoChunks() {
		...
		const rl = readline.createInterface({
			...
			input: fs.createReadStream(this.entryFile, { highWaterMark: Math.ceil(this.heatSize * 0.2) }),
			crlfDelay: Infinity,
		});
		...
		for await (const line of rl) {
			size += line.length;
			if (size >= this.heatSize) {
				this._WriteIntoTmpFile(data, ti++, comparer);
				size = 0;
				data = [];
			}
			data.push(line);
		...
		if (data.length > 0) await this._WriteIntoTmpFile(data, ti++);
	}
	...
	async _MergeSortedFiles() {
		...
		const writer = fs.createWriteStream(this.outputFile, { highWaterMark: Math.ceil(this.heatSize * 0.1) });
	...
	async sort() {
		...
		await this._MergeSortedFiles(config);
